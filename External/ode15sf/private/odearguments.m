...
%   $Revision: 1.12.4.11 $  $Date: 2008/06/20 08:00:32 $
...
if FcnHandlesUsed  % function handles used
  if isempty(tspan) || isempty(y0) 
    error('MATLAB:odearguments:TspanOrY0NotSupplied',...
          ['When the first argument to %s is a function handle, ',...'
           'the tspan and y0 arguments must be supplied.'], solver);
  end      
  if length(tspan) < 2
    error('MATLAB:odearguments:SizeTspan',...
          ['When the first argument to %s is a function handle, ',...'
           'the tspan argument must have at least two elements.'], solver);
  end  
  ...
  if ~( isempty(options) || isa(options,'struct') )
    if (length(tspan) == 1) && (length(y0) == 1) && (min(size(options)) == 1)
      ...
      error('MATLAB:odearguments:IncorrectSyntax',...
            'Correct syntax is %s(%s,tspan,y0,options).',...
            solver,funstring(ode));      
    end
  end   
  ...
  if isempty(tspan) || isempty(y0) 
    if exist(ode)==2 && ( nargout(ode)<3 && nargout(ode)~=-1 ) 
      ...
      error('MATLAB:odearguments:NoDefaultParams',...
            'No default parameters in %s. Use %s(%s,tspan,y0,...) instead.',...
...
if n > 1
  error('MATLAB:odearguments:FoMustReturnCol',...
        '%s must return a column vector.',funstring(ode));
elseif m ~= neq
  error('MATLAB:odearguments:SizeIC', ...
        'Solving %s requires an initial condition vector of length %d.',...
        funstring(ode),neq);
end
...
if normcontrol
  ...
  if (length(atol) ~= 1) && (length(atol) ~= neq)
    error('MATLAB:odearguments:SizeAbsTol',...
          'Solving %s requires a scalar AbsTol, or a vector AbsTol of length %d',...
           funstring(ode), neq); 
  end
  atol = atol(:);
  normy = [];
