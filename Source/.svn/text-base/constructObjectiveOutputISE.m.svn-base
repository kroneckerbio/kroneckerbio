...
%% update function
    function objNew = update(mNew)
        objNew = constructObjectiveOutputISE(mNew, outputIndex, data);
    end
    ...
    function val = dgdx(t,x,u,nSim)
        val = 2*(m.c(outputIndex, :)*x - data(t)).'*m.c(outputIndex, :)/N;
    end
    function val = dgdp(t,x,u,nSim)
        val = zeros(1, m.nP);
    end
    function val = d2gdx2(t,x,u,nSim)
        val = 2*m.c(outputIndex, :).'*m.c(outputIndex, :)/N;
    end
