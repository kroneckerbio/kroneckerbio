function ints = integrateAllCurv(m, con, obs, opts, use_finite)
if nargin < 5
    use_finite = false;
end
...
if any([obs.Complex])
    if ~use_finite
        ints = integrateCurvComp(m, con, tF, eve, fin, opts);
    else
        ints = integrateCurvCompFinite(m, con, tF, eve, fin, opts);
    end
    ...
    for i_obs = 1:n_obs
        if obs(i_obs).Complex
            ...
            ints(i_obs).d2xdT2 = ints(i_obs).d2xdT2(ints(i_obs).t);
            ints(i_obs).d2udT2 = ints(i_obs).d2udT2(ints(i_obs).t);
            ints(i_obs).d2ydT2 = ints(i_obs).d2ydT2(ints(i_obs).t);
        end
    end
else
    if ~use_finite
        ints = integrateCurvSimp(m, con, tF, eve, fin, t_get, opts);
    else
        ints = integrateCurvSimpFinite(m, con, tF, eve, fin, t_get, opts);
    end
    ...
    for i_obs = 1:n_obs
        ...
        if numel(ints(i_obs).t) ~= numel(obs(i_obs).DiscreteTimes) % Saves time and memory if solution is large
            ...
            ints(i_obs).d2xdT2 = ints(i_obs).d2xdT2(:,inds_i);
            ints(i_obs).d2udT2 = ints(i_obs).d2udT2(:,inds_i);
            ints(i_obs).d2ydT2 = ints(i_obs).d2ydT2(:,inds_i);
        end
    end
end
...
for i_obs = 1:n_obs
    ...
    ints(i_obs).d2xedT2 = ints(i_obs).dxedT(:,current_events);
    ints(i_obs).d2uedT2 = ints(i_obs).duedT(:,current_events);
    ints(i_obs).d2yedT2 = ints(i_obs).dyedT(:,current_events);
end
