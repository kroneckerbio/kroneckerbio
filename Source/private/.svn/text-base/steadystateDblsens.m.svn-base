...
%         fqUseControls = vec(fqUseControls(:,opts.UseControls{1})); % indexes that remove inactive q from fq_
        ...
        function val = derivative(t, joint, u)
            ...
            temp = sparse(d2fdxdT(-1,x,u) * dxdT); % fT_x * x_T -> fT_T
            ...
            d2xdT2dot = reshape(dfdx(-1,x,u) * d2xdT2, nx*nT,nT) + d2xdT2dot + temp + d2fdT2(-1,x,u); % (f_x * x_TT -> f_TT -> fT_T) + fT_T + fT_T + fT_T -> fT_T
        ...
        function val = jacobian(t, joint, u)
            ...
            % Compute d/dx(dfdT)
            d2fdxdTVal = sparse(d2fdx2(-1,x,u) * dxdT) + d2fdTdx(-1,x,u); % fx_T
            ...
            d3fddxdTT2 = d3fddxdTT2 + d2fdxdT(-1,x,u); % fT_x + fT_x -> fT_x
