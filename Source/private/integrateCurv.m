function sol = integrateCurv(m, con, opts)
% Constants
nx = m.nx;
nu = m.nu;
nk = m.nk;
...
sol = accumulateOdeFwd(der, jac, 0, con.tF, ic, u, con.Discontinuities, 1:nx, opts.RelTol, opts.AbsTol(1:nx+nx*nT+nx*nT*nT));
sol.u = u;
sol.C1 = m.C1;
sol.C2 = m.C2;
sol.c  = m.c;
...
%         fqUseControls = vec(fqUseControls(:,opts.UseControls{1})); % indexes that remove inactive q from fq_
        ...
        function val = d2fdT2Sub(t, x, u)
            ...
            val = [val(fkUseParams,opts.UseParams), sparse(nx*nTk, nTs+nTq), spermute132(d2fdkdq, [nx,nTq,nTk], [nx*nTk,nTq]);
                   sparse(nx*nTs, nTk+nTs+nTq);
                   d2fdkdq,                         sparse(nx*nTq, nTs),     d2fdq2];
        end
        ...
        function val = d2fdTdxSub(t, x, u)
            ...
            val = [val(:,opts.UseParams), sparse(nx*nx, nTs), d2fdqdx(:,opts.UseControls)]; % fx_T
        end
