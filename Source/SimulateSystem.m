...
sim = emptystruct([n_obs,n_obs], 'Type', 'Name', 't', 'x', 'u', 'y', 'ie', 'te', 'xe', 'ue', 'ye', 'more', 'int');
...
for i_con = 1:n_con
    ...
    for i_obs = 1:n_obs
        sim(i_obs,i_con) = pastestruct(sim(i_obs,i_con), obs(i_obs,i_con).Simulation(ints(i_obs)));
    end
end
